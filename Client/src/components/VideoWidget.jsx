...
export default function VideoWidget({ video, onUpdate, onDelete, zoom = 1, panOffset = { x: 0, y: 0 }, userRole }) {
  ...
  const handlePlayPause = (e) => {
    ...
    onUpdate(video.id, { isPlaying: next, currentTime: videoRef.current?.currentTime || 0 });
  };
  const handleSeek = (e) => {
    e.stopPropagation();
    ...
    setCurrentTime(t);
    onUpdate(video.id, { currentTime: t });
  };
  ...
  const handleDelete = (e) => { e.stopPropagation(); if (onDelete) onDelete(video.id); };
  ...
  return (
    ...
    >
      ...
      <div onPointerDown={startDrag}
        ...
        {isHost && <button onClick={handleDelete} style={{ background: "none", border: "none", color: "#ef4444", fontSize: 14, cursor: "pointer", padding: "0 2px", lineHeight: 1 }} title="ลบวิดีโอ">✕</button>}
      </div>

      {/* Video */}
      <video ref={videoRef} src={video.url} preload="metadata"
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent, rgba(0,0,0,0.85))",
        ...
        <div ref={progressRef} onClick={handleSeek} style={{ width: "100%", height: 5, background: "rgba(255,255,255,0.2)", borderRadius: 3, cursor: isHost ? "pointer" : "default", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#fff", fontSize: sw > 350 ? 12 : 10 }}>
          <button onClick={handlePlayPause} style={{ background: "none", border: "none", color: "#fff", fontSize: sw > 350 ? 16 : 13, cursor: isHost ? "pointer" : "not-allowed", opacity: isHost ? 1 : 0.5, padding: "1px 3px" }}>
            {isPlaying ? "⏸" : "▶"}
          </button>
      ...
      {!isPlaying && !showControls && (
        <div onClick={handlePlayPause} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: 52, height: 52, borderRadius: "50%",
          ...
          cursor: isHost ? "pointer" : "not-allowed", border: "2px solid rgba(255,255,255,0.3)" }}>
          <span style={{ fontSize: 22, color: "#fff", marginLeft: 3 }}>▶</span>
        </div>
      )}

      {/* Resize handles (only when editing) */}
      {editing && ["nw","ne","sw","se"].map(c => (
